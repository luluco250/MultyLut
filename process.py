...
TECHNIQUE_NAME_REGEX = re.compile(r"technique\s+(\S+)")
...
def process_file(path):
	...
	with open(path, "r") as input, open(output_path, "w") as output:
		...
		try:
			technique_name = find_keyword(in_txt, TECHNIQUE_NAME_REGEX)
			out_lines.append(f"#define fLUT_TechniqueName {technique_name}")
